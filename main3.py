...
#Amanha verificar se tem um maximo para comprar/vender; Verificar se sempre ao iniciar esta vendendo se sim bloquear isso(so voltar != 0); Criar um arquivo txt em toda operaÃ§Ã£o tendo valor inicial antes de tudo das duas moedas, valor da compra/venda, e o lucro sempre q vender
...
rsi_anterior = None
nome_arquivo_final = ''
#FunÃ§Ã£o para criar um txt com informaÃ§Ãµes
def criar_arquivo_saldo(saldo_inicial,saldo_final,lucro_perda,cripto):
    ...
    with open(caminho_arquivo, modo_abertura) as arquivo:
        if modo_abertura == 'w':
            arquivo.write("RelatÃ³rio diÃ¡rio\n")
...
# FunÃ§Ã£o para adicionar uma linha ao log txt
...
def calcular_indicadores(df):
    ...
    # MACD (12, 26, 9 sÃÂ£o os perÃÂ­odos padrÃÂ£o)
...
def comprar_mercado(symbol, quantidade,compra_agora,tentativas=3):
    ...
    while tentativa_atual <= tentativas:
        try:
            ...
            print(f"Saldo BRL: {saldo_usdc} BRL (DisponÃ­vel) na Compra")
            
            saldo_sei = obter_saldo_crypt('SOL')
            print(f"Saldo SOL: {saldo_sei} SOL (DisponÃ­vel) na Compra")
            ...
            if float(ordem['executedQty']) == 0.0:
                # preco_compra = 0.0
                adicionar_linha_ao_arquivo(f"Ordem de compra nao realizada pois Ã© zero: {ordem['executedQty']}")
                tentativa_atual += 1
                time.sleep(1)
            ...
            print(f"Saldo BRL: {saldo_usdc_pos} BRL (DisponÃ­vel) Pos compra")
            
            saldo_sei_pos = obter_saldo_crypt('SOL')
            print(f"Saldo SOL: {saldo_sei_pos} SOL (DisponÃ­vel) Pos compra")
            criar_arquivo_saldo(saldo_usdc,saldo_usdc_pos,f'Gastei {valores_investidos['total_gasto']}' ,'BRL')
            
            criar_arquivo_saldo(saldo_sei,saldo_sei_pos,f'Para Comprar {valores_investidos['total_quantidade_comprada']}','SOL')
            
            print(f' Na compra o comprou Ã©: {comprou} e vendeu Ã©: {vendeu}')
            return True
...
# FunÃ§Ã£o para realizar uma ordem de venda de mercado (Market Sell)
    ...
    while tentativa_atual <= tentativas:
        try:
            ...
            if float(saldo_sei) < 0.0001000000:
                adicionar_linha_ao_arquivo(f"NÃ£o vendeu pois o saldo Ã© menor do que o permitido: {saldo_sei}")
                return False
            print(f"Saldo SOL: {saldo_sei} SOL (DisponÃ­vel) na Venda")
            saldo_usdc = obter_saldo_crypt('BRL')
            print(f"Saldo BRL: {saldo_usdc} BRL (DisponÃ­vel) na Venda")
            ...
            if float(ordem['executedQty']) == 0:
                # preco_venda = 0.0
                adicionar_linha_ao_arquivo(f"Ordem de venda nao realizada pois Ã© zero: {ordem['executedQty']}")
                tentativa_atual += 1
                time.sleep(1)
                continue  # Tentar novamente
            ...
            saldo_usdc_pos = obter_saldo_crypt('BRL')
            print(f"Saldo BRL: {saldo_usdc_pos} BRL (DisponÃ­vel) Pos venda")
            saldo_sei_pos = obter_saldo_crypt('SOL')
            print(f"Saldo SOL: {saldo_sei_pos} SOL (DisponÃ­vel) Pos venda")
            
            criar_arquivo_saldo(saldo_sei,saldo_sei_pos,F'Vendi {valores_ganhos['total_quantidade_vendida']}','SOL')
            criar_arquivo_saldo(saldo_usdc,saldo_usdc_pos,f'Para receber {valores_ganhos['total_ganho']}','BRL')
            vendeu = vendeu +1
            if comprou != 0:
                comprou = comprou -1
            print(f' Na venda o comprou Ã©: {comprou} e vendeu Ã©: {vendeu}')
            return True
        except Exception as e:
            print(f"Erro ao realizar a ordem de venda: {e}")
            return False  # ApÃ³s vÃ¡rias tentativas, falha na venda
        return False
...
# FunÃ§Ã£o para calcular lucro ou perda
def calcular_lucro_perda(compra, venda):
    ...
    # Calcular o valor lÃ­quido da venda (deduzindo a taxa)
    ...
    # Calcular o custo lÃ­quido da compra (deduzindo a taxa)
...
def toma_decisao(df):
    ...
    ultimo_precos = df.iloc[-1]  # Pega os Ãºltimos valores

    # VariÃ¡veis baseadas nos Ãºltimos preÃ§os
    ...
    print(ultimo_precos)
    # 1. CondiÃ§Ã£o de Compra
    # Comprar se o preÃ§o estiver abaixo da banda inferior de Bollinger e RSI < 30 and ema_9 > ema_21
    if close_price <= bollinger_middle and (rsi < 30 or 30 <= rsi <= 40)  and macd_valor_atual > macd_signal_atual:
        rsi_anterior = rsi
        print(f"PreÃ§o abaixo da banda inferior de Bollinger RSI entre 30 e 40. Considerar compra.")
        adicionar_linha_ao_arquivo(f"PreÃ§o abaixo da banda inferior de Bollinger RSI entre 45 e 60. Considerar compra.")
        return True
    elif rsi < 30 and close_price < bollinger_lower  and macd_valor_atual > macd_signal_atual: #
        print(f"RSI muito baixo e preÃ§o abaixo da banda inferior, MACD sugere reversÃ£o. Considerar compra.")
        adicionar_linha_ao_arquivo(f"RSI muito baixo, preÃ§o abaixo da banda inferior, MACD sugere reversÃ£o. Considerar compra.")
        return True
    elif close_price > bollinger_upper:
        print(f"PreÃ§o estÃ¡ acima da banda superior. Considerar esperar.")
        adicionar_linha_ao_arquivo(f"PreÃ§o estÃ¡ acima da banda superior. Evitar compra.")
        return 'Esperar'
    ...
    # if rsi < 30:
    #     adicionar_linha_ao_arquivo(f"RSI < 30 mas bollinger nao Ã© maior que o close_price. Considerar venda.")
    #     rsi_anterior = rsi
    #     return True
    # 2. CondiÃ§Ã£o de Venda
    # Vender se o preÃ§o estiver acima da banda superior de Bollinger e RSI > 70 and ema_9 < ema_21
    if close_price >= bollinger_middle and rsi >= 65 :
        rsi_anterior = rsi
        print(f"PreÃ§o acima da banda superior de Bollinger e RSI > 65. Considerar venda.")
        adicionar_linha_ao_arquivo(f"PreÃ§o acima da banda superior de Bollinger e RSI > 65. Considerar venda.")
        return False
    
    # if rsi > 75:
    #     rsi_anterior = rsi
    #     adicionar_linha_ao_arquivo(f"RSI > 75 mas bollinger nao Ã© menor que o close_price. Considerar venda.")
    #     return False
...
def main_bot():
    ...
    if ultimo_valor == 0:
        ...
        print(f"PreÃ§o Atual: {valor_inicial} INICIO")
        ultimo_valor = valor_inicial
        print('Pegando primeiro valor')
        saldo_usdc = obter_saldo_crypt('BRL')
        adicionar_linha_ao_arquivo(f'Valor de BRL inicial: {saldo_usdc}')
        print(f"Saldo BRL: {saldo_usdc} BRL (DisponÃ­vel)")
        saldo_sei = obter_saldo_crypt('SOL')
        adicionar_linha_ao_arquivo(f'Valor de SOL inicial: {saldo_sei}')
        print(f"Saldo SOL: {saldo_sei} SOL (DisponÃ­vel)")
    else:
        print('iniciando processo')
        valor_agora,step_size_symbol = pega_info_preco()
        print(f"PreÃ§o Atual: {valor_agora}")
        ultimo_precos = historico.iloc[-1]  # Pega os Ãºltimos valores
        
        # VariÃ¡veis baseadas nos Ãºltimos preÃ§os
        rsi = ultimo_precos['rsi']
        macd_atual = ultimo_precos['macd_line']
        macd_signal = ultimo_precos['macd_signal']
        print(f'RSI {rsi}, ultimo_valor Ã©: {ultimo_valor} e valor_agora Ã©: {valor_agora} e preco_compra Ã©: {preco_compra} e preco venda Ã© {preco_venda}')
        adicionar_linha_ao_arquivo(f'RSI {rsi}, ultimo_valor Ã©: {ultimo_valor} e valor_agora Ã©: {valor_agora} e preco_compra Ã©: {preco_compra} e preco venda Ã© {preco_venda}')
        ...
        if float(valor_agora) <= stop_loss and preco_compra != 0.0 and macd_atual < macd_signal and rsi < 30:
            adicionar_linha_ao_arquivo(f'PreÃ§o caiu demais melhor vender agora, stop loss {stop_loss} valor agora{valor_agora}')
            # saldo_usdc = obter_saldo_crypt('BRL')
            # print(f"Saldo BRL: {saldo_usdc} BRL (DisponÃ­vel)")
            # saldo_sei = obter_saldo_crypt('SOL')
            # print(f"Saldo SOL: {saldo_sei} SOL (DisponÃ­vel)")
        ...
        if decisao == True :
            ...
            if valor_total_compra < 10.00:
                adicionar_linha_ao_arquivo(f'PreÃ§o menor do que R$10,00 {valor_total_compra}')
                # Calcula a quantidade mÃ­nima necessÃ¡ria para atingir R$10,00
                ...
                while valor_total_compra_ajustado < 10.00:
                    # Incrementa a quantidade para atingir o valor mÃ­nimo
                    quantidade_compra += step_size_symbol
                    quantidade_compra = ajustar_quantidade(quantidade_compra, step_size_symbol)
                    valor_total_compra_ajustado = quantidade_compra * float(valor_agora)
                # Ajusta a quantidade mÃ­nima com base no step_size
            ...
            if comprar_final != False:
                # Armazenar o preÃ§o de compra e a quantidade comprada
                preco_compra = valor_agora
                ultimo_valor = valor_agora
                quantidade_comprada = quantidade_compra
            # else:
            #     adicionar_linha_ao_arquivo(f'Nao comprou pois variaÃ§Ã£o {variacao_aceitavel_agora} e variaÃ§Ã£o de compra {variacao_aceitavel_agora_compra} e deve comparar preÃ§o de compra {deve_comparar_preco_compra} ')
        ...
        # elif decisao == False and float(ultimo_valor) < float(valor_agora) and float(preco_compra) < float(valor_agora):
            ...
            if float(valor_agora) >= float(preco_minimo_venda) :
                ...
                adicionar_linha_ao_arquivo(f'Valor de tentativa Ã© de {valor_venda}')
                if float(valor_min_venda) < 10.00 :
                    ...
                    adicionar_linha_ao_arquivo(f'Novo Valor de tentativa Ã© de {valor_venda}')
                    print(f'Valor de tentativa Ã© de {valor_venda}')
                ...
                adicionar_linha_ao_arquivo(f"PreÃ§o de venda ({valor_agora}) ainda nÃ£o atingiu a margem mÃ­nima de lucro ({preco_minimo_venda}), NÃ£o vendeu")    
        else:
            print('NÃ£o fiz nada')
        ultimo_valor = valor_agora
